...
public:
    ...
    void rotate();
    // Remove the first node and put it at the end

    void deleteKthNode(int num);
    // Remove the Kth node
    // Can assume always positive
...
void linkedListType<Type>::rotate()
{
  ...
  if(first == nullptr)
  {
    cout << "Cannot swap nodes" << endl;
  }
  else
  {
    nodeType<Type> *OF = first;
    first = first->link;
    nodeType<Type> *NF = first;
    while(NF->link){
      NF = NF->link;
    }
    NF->link = OF;
